...
async.mapSeries(jD, function(Tunnel, _cb) {
    var job = queue.create('Tunnel Speed Report', Tunnel).priority('low').attempts(5).backoff( {type:'exponential'} ).searchKeys(['IP', 'tunnel', 'CountryLong', 'CountryShort', 'file']).save(function(err) {
        if (err) throw err;
